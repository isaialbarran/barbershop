...
export default function ListReviews(props) {
  const { navigation, idBarbershop, setRating } = props;
  ...
  useEffect(() => {
    (async () => {
      ...
      db.collection('reviews')
        .where('idRestaurant', '==', idBarbershop)
        .get()
        .then((response) => {
    ...
    <View>
      {userLogged ? (
        <Button
          ...
          onPress={() =>
            navigation.navigate('AddReviewBarbershop', {
    ...
    <View style={styles.viewReview}>
      <View style={styles.viewImageAvatar}>
        <Avatar
          ...
          source={{
            url: avatarUser ? avatarUser : 'https://api.adorable.io/avatars/285/abott@adorable.png',
          }}
        />
      </View>
